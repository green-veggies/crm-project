...
import {
  ...
  Typography, TextField, createTheme, ThemeProvider
} from '@mui/material';
...
const ContactsTablePage = () => {
  ...
  const handleEditSubmit = async () => {
    try {
      const response = await fetch(`http://localhost:8800/api/contacts/${selectedContact._id}`, {
        method: 'PUT',
      ...
      if (response.ok) {
        setContacts((prev) =>
          prev.map((contact) => (contact._id === selectedContact._id ? selectedContact : contact))
        );
        handleEditClose();
      } else {
  ...
  const sortedContacts = contacts.slice().sort((a, b) => {
    ...
    // For numeric or other types of sorting (like phone numbers)
